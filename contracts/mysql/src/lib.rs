...
const RECORD_SIZE: usize = 57; // hash (8) + amount(8) + claimed (1) + time (8) + sender (32)
...
fn add_hash(_program_id: &Pubkey, accounts: &[AccountInfo], rounds_data: &[u8], data: &[u8]) -> ProgramResult {
    ...
    for record in storage_data_ff.chunks_exact_mut(RECORD_SIZE) {
        if record[..8] == hash[..] {
            ...
            if record[16] == 0 {
                let expected_sender_account = Pubkey::new_from_array(record[25..57].try_into().unwrap());
                ...
                record[17..25].copy_from_slice(&current_play.to_le_bytes());
    ...
    if create == true{
        for record in storage_data_ff.chunks_exact_mut(RECORD_SIZE) {
            if record[8..].iter().all(|&x| x == 0) {
                ...
                let current_play = Clock::get().map_err(|_| ProgramError::InvalidAccountData)?.unix_timestamp;
                record[17..25].copy_from_slice(&current_play.to_le_bytes());
                record[25..57].copy_from_slice(sender_account.key.as_ref()); 
...
fn refund_amount(
    ...
    for (i, record) in storage_data_ff.chunks_exact(RECORD_SIZE).enumerate() {
        if record[..8] == hash[..8] {
            ...
            let sended_day = u64::from_le_bytes(record[17..25].try_into().unwrap());
            ...
            amount = u64::from_le_bytes(record[8..16].try_into().unwrap());
            expected_sender_account = Pubkey::new_from_array(record[25..57].try_into().unwrap());
            record_index = Some(i);
            break;
        }
    }
